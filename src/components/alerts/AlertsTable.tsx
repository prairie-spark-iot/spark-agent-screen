...
export const AlertsTable = React.memo(function AlertsTable({
  ...
  // Internal pagination slicing
  const start = (page - 1) * pageSize;
  const pageAlerts = React.useMemo(() => filteredAlerts.slice(start, start + pageSize), [filteredAlerts, page, pageSize]);
    ...
    <div className="bg-[#141822] border border-[#2d3240] rounded-xl overflow-hidden shadow-md flex flex-col">
        ...
        <Table className="w-full text-left border-collapse whitespace-nowrap text-xs border-0">
          <TableHeader className="bg-[#1a1f2c] border-b border-[#2d3240] hover:bg-transparent">
            <TableRow className="font-mono text-[9px] font-bold tracking-wider text-white/90 uppercase border-b-0 hover:bg-transparent">
              <TableHead className="px-3 py-3 w-10 h-auto border-b-0 text-[#849495] text-center">#</TableHead>
          ...
          <TableBody className="divide-y divide-[#2d3240] border-0">
            {filteredAlerts.length === 0 ? (
              <TableRow className="hover:bg-transparent">
                <TableCell colSpan={8} className="px-5 py-8 text-center text-[#d1d5db]/50 font-sans border-0">
                  {t('noAlertsFound')}
                </TableCell>
              </TableRow>
            ) : pageAlerts.length === 0 ? (
              ...
              <>
                {pageAlerts.map((alertRaw, index) => {
                  ...
                  >
                    <TableCell className="px-3 py-4 font-mono text-[#849495] text-center border-0 text-[11px] tabular-nums">
                ...
                {pageAlerts.length < pageSize && pageAlerts.length > 0 && (
                  Array.from({ length: pageSize - pageAlerts.length }, (_, i) => (
                    <TableRow key={`spacer-${i}`} className="hover:bg-transparent cursor-default">
                      <TableCell colSpan={8} className="px-5 py-4 border-0">&nbsp;</TableCell>
                    </TableRow>
                  ))
                )}
              </>
      ...
      <div className="p-4 border-t border-[#222630] flex items-center justify-between bg-[#191c23] text-xs">
        <p className="text-[#b9cacb]">
          {t('showing')}{' '}
          <span className="text-white font-mono">{pageAlerts.length}</span>{' '}
          {t('ofTotal')}{' '}
