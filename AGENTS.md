...
src/                  Client-side application
  ...
  components/         UI per-feature: layout/, alerts/, dashboard/, devices/, knowledge/, diagnosis/
...
- **Device drift simulation**: A 5-second `setInterval` in the server process drifts device telemetry values and auto-generates new alerts when thresholds are exceeded (closed-loop simulation).
...
- **Auth**: Client-side only. `admin / admin123456`, stored in `localStorage` key `spark_iot_auth`. No real auth.
...
- **Bundle**: `pnpm-lock.yaml` is the lockfile. `npm install` will not match. Always use `pnpm`.
...
- Dev server reads `.env.local` for `GEMINI_API_KEY`. Without it, AI diagnosis falls back to client-side heuristics.
...
- The `lib/db.ts` drift simulation should be considered when debugging — it mutates device values and auto-creates alerts every 5 seconds.
